...
export const useUpdateMenuItem = () => {
    ...
    return useMutation({
        ...
        onSuccess: (data, variables) => {
            ...
            queryClient.setQueryData(menuQueryKeys.detail(variables.id), data);
        },
        onError: (error) => {
...
// Hook để import menu items từ Excel
export const useImportMenuItems = () => {
    const queryClient = useQueryClient();

    return useMutation({
        mutationFn: async (file: File): Promise<BaseResponse<void>> => {
            return await menuApis.importMenuItems(file);
        },
        onSuccess: () => {
            // Invalidate và refetch danh sách menu
            queryClient.invalidateQueries({ queryKey: menuQueryKeys.lists() });
        ...
        onError: (error) => {
            console.error("Lỗi khi import menu items:", error);
        },
    });
};
