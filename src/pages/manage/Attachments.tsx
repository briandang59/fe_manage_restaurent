...
import { useAttachments, useUploadAttachment, useDeleteAttachment } from "@/utils/hooks/useAttachments"
import { Upload, File, Image, Trash2, Download, Eye, ChevronLeft, ChevronRight } from "lucide-react"
...
function Attachments() {
  ...
  const handleDownload = async (file: AttachmentResponse) => {
    ...
    const isExternalUrl = displayUrl.includes('cloudinary.com') || 
    ...
    <div className="space-y-6 bg-white p-4 rounded-lg shadow-md">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
        {attachments.map((file) => (
          <div key={file.id} className="group relative bg-gray-50 rounded-lg p-3 hover:bg-gray-100 transition-colors">
            {/* File Preview */}
            <div className="aspect-square bg-white rounded-md flex items-center justify-center mb-2 overflow-hidden">
              ...
              <div className={`${isImageFile(file.file_name) ? 'hidden' : ''} flex items-center justify-center`}>
            ...
            {/* File Info */}
               ...
               <p className="text-sm font-medium text-gray-900 truncate" title={file.file_name || 'Unknown file'}>
                 {file.file_name || 'Unknown file'}
               </p>
              <p className="text-xs text-gray-500">
                {formatFileSize(file.size || 0)} {/* Using price field as file size temporarily */}
              </p>
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>
                        Bạn có chắc chắn muốn xóa file "{file.file_name || 'Unknown file'}"? Hành động này không thể hoàn tác.
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
            ...
            <DialogTitle>Xem trước file</DialogTitle>
             ...
             <div className="space-y-4">
               <div className="aspect-video bg-gray-100 rounded-lg flex items-center justify-center overflow-hidden">
                 ...
                 <div className={`${isImageFile(selectedFile.file_name) ? 'hidden' : ''} flex items-center justify-center`}>
                   {getFileIcon(selectedFile.file_name)}
                 </div>
               </div>
                             <div className="space-y-2">
                 <p><strong>Tên file:</strong> {selectedFile.file_name || 'Unknown file'}</p>
                <p><strong>Kích thước:</strong> {formatFileSize(selectedFile.size || 0)}</p>
                <p><strong>Ngày tạo:</strong> {formatDate(selectedFile.created_at)}</p>
                <p><strong>Mô tả:</strong> {selectedFile.mime_type || "Không có mô tả"}</p>
              </div>
                             <div className="flex gap-2">
                 <Button 
                   variant="outline" 
